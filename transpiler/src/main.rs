    ...
    for file in [
        ...
        run_command(
            ...
            &[
                "quicktype",
                "--src",
                &format!("output/{file}.json"),
                "--lang",
                "rust",
                "--out",
                &format!("../api/src/types/{file}.rs"),
            ],
            &format!("Run QuickType on {file}.json"),
            Some("workdir"),
        )?;

        // fix rust files
        {
            let ty = fs::read_to_string(format!("api/src/types/{file}.rs"))
                .map_err(|e| Error::Err(e.to_string()))?
                .replace("\n\n    ", "\n    ");
            ...
            fs::write(format!("api/src/types/{file}.rs"), ty)
                .map_err(|e| Error::Err(e.to_string()))?;
        }
    }
    run_command("cargo", &["fmt"], "cargo fmt", None)?;
